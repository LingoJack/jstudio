...
/// Read a single document by id from `documents/{doc_id}.json`.
#[tauri::command]
pub fn read_document(doc_id: String) -> Result<Value, String> {
    let path = documents_dir().join(format!("{doc_id}.json"));
    let data =
...
/// Write a single document to `documents/{doc_id}.json` (pretty-printed).
#[tauri::command]
...
pub fn delete_document(doc_id: String) -> Result<(), String> {
    let path = documents_dir().join(format!("{doc_id}.json"));
    if path.exists() {
        fs::remove_file(&path).map_err(|e| format!("failed to delete document {doc_id}: {e}"))?;
    }
    Ok(())
}

/// Save a binary asset to `assets/{asset_id}.{ext}`. Returns the file name.
#[tauri::command]
...
/// List all assets in the assets directory with metadata.
#[tauri::command]
pub fn list_assets() -> Result<Vec<Value>, String> {
    ...
    for entry in read.flatten() {
        ...
        // Derive display name and type from file name.
...
fn format_file_size(bytes: u64) -> String {
    ...
    let i = (bytes as f64).log2() / 10.0; // log_1024
