...
test("forward: text block round-trips", () => {
  const block = mkBlock("text", [
    { text: "hello", annotations: { bold: true } },
  ]);
...
test("forward: quote round-trips", () => {
  const block = mkBlock("quote", [
    { text: "quoted", annotations: { italic: true } },
  ]);
  const result = tiptapJSONToOurBlock(ourBlockToTiptapJSON(block));
  assert.deepEqual(normalizeBlock(result), normalizeBlock(block));
});
...
test("forward: empty code block does not throw", () => {
  const block = mkBlock("code", [{ text: "", annotations: {} }], {
    language: "plaintext",
  });
...
test("forward: diagram block round-trips", () => {
  ...
  const result = tiptapJSONToOurBlock(ourBlockToTiptapJSON(block));
  assert.equal(
    result.properties?.diagramSnapshot,
    '{"kind":"jgraph","nodes":[]}',
  );
...
test("REGRESSION: table cell with orderedList inside survives via rawContent", () => {
  ...
  assert.equal(
    backCell?.content?.[0]?.content?.[0]?.content?.[0]?.type,
    "paragraph",
  );
...
test("REGRESSION: plain-paragraph table cell does NOT set rawContent", () => {
  ...
  assert.equal(
    cell.rawContent,
    undefined,
    "plain cell must not set rawContent",
  );
...
test("REGRESSION: nested list 3+ levels round-trips", () => {
  ...
  assert.equal(
    back.content?.[1]?.content?.[0]?.content?.[1]?.type,
    "bulletList",
  );
  assert.equal(
    back.content?.[1]?.content?.[0]?.content?.[1]?.content?.[0]?.content?.[0]
      ?.content?.[0]?.text,
    "L3",
  );
});
...
test("REGRESSION: richText with all annotation types round-trips", () => {
  const rich: RichText[] = [
    ...
    { text: "x^2 + y^2 = r^2", annotations: { inlineMath: true } },
  ];
...
test("inlineMath segment becomes an inlineMath atom node and round-trips", () => {
  const rich: RichText[] = [
    { text: "Euler: ", annotations: {} },
    { text: "e^{i\\pi} + 1 = 0", annotations: { inlineMath: true } },
    { text: " is beautiful.", annotations: {} },
  ];
  const inline = richTextToTiptapInline(rich);
  // The formula segment becomes an inlineMath node, not text with marks.
  const mathNode = inline.find((n) => n.type === "inlineMath");
  assert.ok(mathNode);
  assert.equal(mathNode.attrs?.latex, "e^{i\\pi} + 1 = 0");
  // Plain neighbours stay text nodes.
...
test("REGRESSION: richText with soft line break (hardBreak) round-trips", () => {
  const rich: RichText[] = [{ text: "line1\nline2", annotations: {} }];
  const inline = richTextToTiptapInline(rich);
