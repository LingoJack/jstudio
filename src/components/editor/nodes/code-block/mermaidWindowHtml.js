function buildMermaidPreviewWindowHtml(svg, isDarkMode) {
  const bg = isDarkMode ? "linear-gradient(135deg, #1e1e1e 0%, #252526 100%)" : "linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%)";
  const btnBg = isDarkMode ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.06)";
  const btnHover = isDarkMode ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.12)";
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
